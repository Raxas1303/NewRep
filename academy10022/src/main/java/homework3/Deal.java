...
public class Deal {
	...
	public void deal() {
		if (Regex.test(timeDate) == true) {
			System.out.println("Дата сделки прошла проверку");
		} else if (Regex.test(timeDate) == false) {
		...
		if (buyer.getMoney() < fullPrice) {
			...
			for (int i = 0; i < products.length; i++) {
				if (products[i] != null) {
					System.out.println(i + 1 + ". " + products[i].getName() + "    " + products[i].getQuantity()
							+ "шт   " + products[i].getCalculatePrice() + "$");
				}
			}
			System.out.println("Итого: " + fullPrice + "$");
			System.out.println("--------------------------");
			System.out.println("Стало денег у продавца: " + seller.sellerMoney(fullPrice) + "$");
			System.out.println("Стало денег у покупателя: " + buyer.buyerMoney(fullPrice) + "$");
		}
	}
